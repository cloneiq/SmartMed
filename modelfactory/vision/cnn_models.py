...
def create_model(model_name, pretrained=False, chan_attn=False, spat_attn=False, num_classes=None):
    if model_name.lower() == 'vgg19':
        _model = VGG19(pretrained=pretrained)
    # ResNet
    elif model_name.lower() == 'resnet18':
        _model = ResNet(model_name, pretrained=pretrained,
                        chan_attn=chan_attn, spat_attn=spat_attn,
    ...
    # DenseNet
        ...
        _model = DenseNet161(pretrained=pretrained)
    elif model_name.lower() == 'densenet169':
        _model = DenseNet169(pretrained=pretrained)
    elif model_name.lower() == 'densenet201':
        _model = DenseNet201(pretrained=pretrained)
    return _model


if __name__ == "__main__":
    cnn_model = create_model('resnet50', pretrained=False,
                             chan_attn=True, spat_attn=True,num_classes=14)
    print(cnn_model)
