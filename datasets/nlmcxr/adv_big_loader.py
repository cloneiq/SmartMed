...
class AdvBigLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     # 图像尺寸发生变化（）
        #     trf_images = self.transform(org_image)
        # 获取图像对应的报告全文
        try:
...
def collate_fn(data):
    ...
    org_image, image_name, label, captions, sentence_num, max_word_num = zip(*data)
    ...
    return org_image, image_name, torch.Tensor(label), targets, prob


def dsLoader(mode='train', media_type='jpg', transform=None, batch_size=64, s_max=10, n_max=50,
             shuffle=False):
    ds = AdvBigLoader(mode=mode, media_type=media_type, s_max=s_max, n_max=n_max)
