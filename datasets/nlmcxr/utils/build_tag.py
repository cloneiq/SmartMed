class Tag(object):
    def __init__(self):
        ...
        # 将标签转换为List，即为每一个标签分配一个ID，建立编号与标签的对应关系如{0:'cardiac monitor',1:'lymphatic diseases',...}
        self.id2tags = self.__load_id2tags()
        # 建立标签->编号的对应关系，即{'cardiac monitor':0,'lymphatic diseases':1,...}
        self.tags2id = self.__load_tags2id()

    # 根据数组中的标签编号获取对应标签，如array[0,3,5]表示输出标签编号为0,3,5的标签
    ...
    # 根据标签列表生成One-Hot编码，编码长度为统计标签个数，对应的标签上设置为1，其余标签为0
    ...
    # 根据数组中是否为1来确定是否输出标签，如array[0,0,1],表示只输出第3个标签
    ...
    # 定义统计标签，原始影像报告中的所有不重复标签（有待验证是否正确）
...
if __name__ == '__main__':
    tags = Tag()
    print(len(tags.static_tags))
    print(tags.id2tags)
    tag = ['cardiac monitor', 'lymphatic diseases']
    print(tags.tags2array(tag))
    arr = [0, 1, 3]
    print(tags.array2tags(arr))
    arr2 = [1, 1, 1]
    print(tags.inv_tags2array(arr2))
    # print(tags.tags2id)
